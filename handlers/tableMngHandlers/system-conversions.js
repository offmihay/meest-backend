...
//         aggregate_sizes: aggregateSizes
// });
//
// };
...
module.exports = async (req, res, next, models) => {
    ...
    if (!body_part) {
        return res.status(400).json({ error: "Missing required parameter: body_part" });
    }

    // Query the database for conversion records for the specified body part
    const systemConversionsRecords = await models.system_conversions.findAll({ where: { body_part: body_part } });

    // Check if the query returned an empty array
    if (systemConversionsRecords.length === 0) {
        return res.status(404).json({ error: "No conversions found for the specified body part" });
    }
